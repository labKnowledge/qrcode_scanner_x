...
export async function POST(request: Request) {
  ...
  try {
    ...
    try {
      ...
      return NextResponse.json(
        { success: false, error: 'Failed to load image. Please check the file format.' },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { success: false, error: 'Failed to process image: ' + (error instanceof Error ? error.message : 'Unknown error') },
      { status: 500 }
    );
  }
} 